...
function admin_login_form($login, $pass){
    global $dbc;
  echo '<form action="admin.php" method="POST">' ;
  echo '<p>Admin Login: <input type="text" name="login" value="' . $login . '"> </p> ' ;
  echo '<p>Password: <input type="text" name="pass" value="' . $pass . '"></p>' ;
  echo '<p><input type="submit"></p>' ;
  echo '</form>' ;
}
function check_login($dbc, $login, $pass){
    $query = 'SELECT first_name, password FROM users WHERE first_name ='.$login.'AND WHERE pass ='.$pass;
    $results = mysqli_query($dbc,$query) ;
    if($results){
